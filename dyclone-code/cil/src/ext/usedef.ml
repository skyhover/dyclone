...
let rec computeDeepUseDefStmtKind ?(acc_used=VS.empty)
                                  ?(acc_defs=VS.empty) 
                                   (sk: stmtkind) : VS.t * VS.t =
  let handle_block b =
    (** PI fixes bug: this function may discard previous varUsed/varDefs;
     * to preserve them, we have to use explicit set union for all s.skind cases. *)
  ...
  | Loop (b, _, _, _) ->
      ...
      (VS.union acc_used u, VS.union acc_defs d)
  | Switch (e, b, _, _) -> 
      let _ = ve e in
      let u, d = !varUsed, !varDefs in
      let u', d' = handle_block b in
