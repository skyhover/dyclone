...
module ForwardsDataFlow = 
  ...
  struct
    ...
    (** Process a statement *)
(* PI: a possible bug: after each processStmt, "s"'s data in stmtStartData is
 * not updated. Although this doesn't affect "s"'s direct successors' values,
 * some stmts of no predecessors may not be right. *)
